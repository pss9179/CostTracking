"""JinaAI auto-instrumentation for OpenTelemetry."""
...
class JinaAIInstrumentor(Instrumentor):
    """Auto-instrumentation for JinaAI embeddings library."""
    ...
    def instrument(self) -> None:
        ...
        try:
            ...
            if hasattr(jinaai, "Client"):
                original_init = jinaai.Client.__init__
                ...
                jinaai.Client.__init__ = patched_init

            self._instrumented = True
        except ImportError:
            import structlog
            logger = structlog.get_logger()
            logger.warning("JinaAI not installed, skipping instrumentation")
        except Exception as e:
            import structlog
            logger = structlog.get_logger()
            logger.warning("Failed to instrument JinaAI", error=str(e))

    def uninstrument(self) -> None:
        """Remove JinaAI instrumentation."""
        self._instrumented = False
    ...
    def _trace_embed(self, original_func: Callable, *args, **kwargs) -> Any:
        ...
        model = kwargs.get("model", "jina-embeddings-v2-base-en")

        with ensure_root_span("jinaai"):
            with tracer.start_as_current_span("api.embed") as span:
                span.set_attribute(API_PROVIDER, "jinaai")
                ...
                try:
                    ...
                    # Estimate tokens (rough approximation), default to 0 if failed
                    prompt_tokens = 0
