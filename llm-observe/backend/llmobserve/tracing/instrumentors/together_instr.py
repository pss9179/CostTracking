"""Together.ai auto-instrumentation for OpenTelemetry."""
...
class TogetherInstrumentor(Instrumentor):
    """Auto-instrumentation for Together.ai library (OpenAI-style API)."""
    ...
    def instrument(self) -> None:
        """Apply Together.ai instrumentation."""
        ...
        try:
            ...
            # Wrap Together client (OpenAI-style)
            if hasattr(together, "Together"):
                original_init = together.Together.__init__

                def patched_init(together_self, *args, **kwargs):
                    original_init(together_self, *args, **kwargs)
                    # Prevents duplicate instrumentation during reloads or multiple setup calls
                    if hasattr(together_self.chat.completions.create, "_instrumented"):
                        return
                    if not hasattr(together_self.chat.completions, "_original_create"):
                        original_create = together_self.chat.completions.create
                        together_self.chat.completions._original_create = original_create
                        wrapped = self._wrap_chat_create(original_create)
                        wrapped._instrumented = True
                        together_self.chat.completions.create = wrapped

                together.Together.__init__ = patched_init

            self._instrumented = True
        except ImportError:
            import structlog
            logger = structlog.get_logger()
            logger.warning("Together.ai not installed, skipping instrumentation")
        except Exception as e:
            import structlog
            logger = structlog.get_logger()
            logger.warning("Failed to instrument Together.ai", error=str(e))

    def uninstrument(self) -> None:
        """Remove Together.ai instrumentation."""
        self._instrumented = False
    ...
    def _trace_chat_call(self, original_func: Callable, *args, **kwargs) -> Any:
        ...
        with ensure_root_span("together"):
            with tracer.start_as_current_span("llm.request") as span:
                span.set_attribute(LLM_PROVIDER, "together")
