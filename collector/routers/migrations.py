...
def fix_clerk_user_id(
    ...
    """
    Fix clerk_user_id mismatch by updating trace_events tenant_id.
    Use this when a user's Clerk ID has changed (e.g., after re-creating account).
    """
    try:
        logger.info(f"Fixing tenant_id in trace_events: {old_clerk_id} -> {new_clerk_id}")
        ...
        with engine.connect() as conn:
            trans = conn.begin()
            try:
                # Update trace_events tenant_id only (users table may have unique constraint)
                events_sql = text("""
                    UPDATE trace_events 
